            ...
            {templates.selected ? (
              ...
              <div className="mt-6 space-y-3">
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground">点击下方任一提示立即开始创建</p>
                </div>
                
                <div className="grid grid-cols-1 gap-2">
                  <button
                    onClick={() => {
                      const prompt = '设计一个现代化的产品推介材料页面，包含产品特点展示区域、客户评价轮播、价格对比表格、在线咨询表单等模块，采用响应式布局支持移动端浏览，添加平滑滚动动画、悬停效果、数字递增动画等交互元素，整体风格简洁专业，色彩搭配突出品牌特色，支持产品图片画廊、视频介绍嵌入等功能';
                      onSendMessage?.(prompt);
                    }}
                    className="w-full p-2 text-xs text-left bg-background hover:bg-muted rounded-md transition-colors border border-border/50 hover:border-primary/30"
                  >
                  ...
                  <button
                    onClick={() => {
                      const prompt = '创建一个现代化的飞书企业服务公司落地页，包含导航栏、Hero区域、服务介绍、团队展示、客户案例、联系方式等完整结构，采用渐变背景、卡片布局、图标动画等现代设计元素，添加统计数字展示、服务流程图、客户logowall、在线预约表单等功能模块，支持深色/浅色主题切换，整体风格专业大气，体现企业实力和专业性。\n具体内容细节：\n公司信息：\n\n公司名称：飞书企业服务解决方案\n口号：重塑企业协作方式，释放团队创造力\n主要业务：为企业提供飞书平台定制化部署、系统集成、培训和运维服务\n\n统计数字展示：\n\n服务企业客户：1000+\n累计用户数：50万+\n项目成功率：98.5%\n客户满意度：4.9/5.0\n\n核心服务介绍：\n\n飞书定制化部署 - 根据企业需求定制工作流、权限管理、数据安全配置\n系统集成服务 - 与CRM、ERP、OA等现有系统无缝对接\n企业培训服务 - 提供管理员培训、用户培训、最佳实践分享\n运维支持服务 - 7*24小时技术支持、性能优化、故障排除\n\n团队展示：\n\n技术总监：张明华，前字节跳动高级架构师，10年企业服务经验\n产品总监：李雨晨，飞书生态认证专家，服务过500+企业客户\n客户成功总监：王志强，企业数字化转型专家，MBA背景\n\n客户案例：\n\n理想汽车：3000人规模，实现全员协作效率提升40%，跨部门沟通成本降低60%\n小米集团：10000+员工，海外本地化部署，支持全球化办公需求\n新东方教育：5000+教师，在线教育场景深度定制，课程管理效率提升3倍\n完美世界：游戏研发团队，项目管理流程优化，产品迭代速度提升50%\n\n服务流程：\n\n需求调研 - 深入了解企业现状和痛点\n方案设计 - 定制化解决方案和实施计划\n系统部署 - 专业团队现场实施和配置\n培训上线 - 全员培训和平滑过渡\n持续优化 - 数据分析和功能迭代\n\n客户Logo墙包含：\n理想汽车、小米、新东方、完美世界、泡泡玛特、元气森林、喜茶、蔚来汽车、bilibili、字节跳动等知名企业标志\n联系方式：\n\n咨询热线：400-888-6666\n企业邮箱：service@feishu-enterprise.com\n公司地址：北京市海淀区中关村软件园\n在线预约表单包含：公司名称、联系人、电话、邮箱、员工规模、具体需求等字段\n\n请确保页面具备响应式设计，在移动端也能完美展示，同时加入微交互动画和视觉特效以提升用户体验。';
                      onSendMessage?.(prompt);
                    }}
                    className="w-full p-2 text-xs text-left bg-background hover:bg-muted rounded-md transition-colors border border-border/50 hover:border-primary/30"
                  >
                  ...
                  <button
                    onClick={() => {
                      const prompt = '开发一个功能完整的后台管理系统界面，包含侧边栏导航、顶部工具栏、数据统计仪表板、表格管理、图表展示等核心功能，支持用户权限管理、数据筛选排序、批量操作、表单验证等实用功能，采用现代化UI设计，包含深色模式、响应式布局、图标库、消息通知、面包屑导航等完善的用户体验元素';
                      onSendMessage?.(prompt);
                    }}
                    className="w-full p-2 text-xs text-left bg-background hover:bg-muted rounded-md transition-colors border border-border/50 hover:border-primary/30"
                  >
                  ...
                  <button
                    onClick={() => {
                      const prompt = '设计一个工作汇报高级演示页面（横屏切换、风格现代感、呈现高级动效），内容如下：\n\n## 理想汽车飞书项目本周交付进展\n\n本周项目整体推进顺利，已完成核心功能模块的开发和测试工作。具体完成情况如下：\n在系统集成方面，已成功完成飞书办公套件与理想汽车现有业务系统的对接，涉及3个核心系统、15个接口的数据同步，数据迁移量达到12万条用户记录。用户权限管理和单点登录功能已通过内测，200名测试用户反馈满意度达95%，预计本周五可正式上线。\n功能交付上，完成了组织架构同步、消息通知推送、以及移动端应用的优化。理想汽车IT总监张总和产品负责人李总在周三的评审会上对界面友好度和操作便捷性表示高度认可，特别称赞了移动端的响应速度提升了40%。\n项目风险管控良好，原定的数据迁移工作提前一天完成，为后续上线争取了缓冲时间。本周与理想汽车技术团队召开了4次协调会议，解决了8个技术难点，双方技术对接人员建立了24小时响应机制，沟通效率明显提升。\n下周计划重点推进全员培训和试运行工作，已安排3场培训会覆盖800名员工，确保系统稳定性。预计项目将按原定时间节点完成最终交付，整体进度超出预期5%。\n目前项目处于收尾阶段，团队士气良好，有信心高质量完成交付目标。';
                      onSendMessage?.(prompt);
                    }}
                    className="w-full p-2 text-xs text-left bg-background hover:bg-muted rounded-md transition-colors border border-border/50 hover:border-primary/30"
                  >
                  ...
                  <button
                    onClick={() => {
                      const prompt = '开发一个智能AI提示词优化器工具，包含提示词输入区域、优化建议显示、效果对比预览、模板库管理等核心功能，支持不同AI模型的提示词格式优化、关键词提取、语义增强、结构调整等功能，添加提示词分析、质量评分、历史记录、一键应用等实用特性，采用分栏布局，左侧输入原始提示词，右侧显示优化后的提示词，提供多种优化策略选择';
                      onSendMessage?.(prompt);
                    }}
                    className="w-full p-2 text-xs text-left bg-background hover:bg-muted rounded-md transition-colors border border-border/50 hover:border-primary/30"
                  >
